...
import { Clock, BookOpen, Trophy, Award, Video, Calendar, ExternalLink, Zap, Code as CodeIcon, ChevronRight, Rocket, Flame, FileText, Plus, Layout, Globe, Bell } from 'lucide-react'
    ...
    async function handleEnableNotifications() {
        try {
            ...
            if (sub) {
                ...
                toast.success('Notifications enabled! You will receive alerts even when the app is closed.')
            }
        } catch (err) {
            console.error('Error enabling notifications:', err)
            toast.error('Failed to enable notifications. Please allow permissions in your browser.')
        }
    }
                        ...
                        <div>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', marginTop: '0.75rem' }}>
                                <div style={{ height: 6, width: 100, background: '#f1f5f9', borderRadius: 3, overflow: 'hidden' }}>
                                    {(() => {
                                        const t        = getTierForXP(stats.xp || 0)
                                        const progress = getLevelProgress(stats.xp || 0)
                                        return <div style={{ height: '100%', width: `${progress}%`, background: stats.rankColor || '#94a3b8', borderRadius: 3 }} />
                                    })()}
                                </div>
