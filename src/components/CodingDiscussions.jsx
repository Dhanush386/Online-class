...
export default function CodingDiscussions({ challengeId, currentCode }) {
    ...
    if (loading && view === 'list') {
        return <div style={{ padding: '2rem', textAlign: 'center', color: '#64748b' }}>Loading discussions...</div>
    }

    if (view === 'create') {
        return (
            <div className="animate-fade-in" style={{ padding: '0.5rem' }}>
                <button onClick={() => setView('list')} style={{ background: 'none', border: 'none', color: '#94a3b8', display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', marginBottom: '1.5rem', fontSize: '0.85rem' }}>
                ...
                <form onSubmit={handleCreateThread} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    <div>
                        ...
                        <input value={newTitle} onChange={e => setNewTitle(e.target.value)} placeholder="E.g. Getting an EOFError on Test Case 2" required style={{ width: '100%', padding: '0.75rem', borderRadius: 8, background: '#0f172a', border: '1px solid #334155', color: '#fff', fontSize: '0.9rem' }} />
                    </div>
                    <div>
                        <label style={{ display: 'block', fontSize: '0.8rem', color: '#94a3b8', marginBottom: '0.5rem' }}>Description</label>
                        <textarea value={newContent} onChange={e => setNewContent(e.target.value)} placeholder="Describe what you need help with..." required rows={5} style={{ width: '100%', padding: '0.75rem', borderRadius: 8, background: '#0f172a', border: '1px solid #334155', color: '#fff', fontSize: '0.9rem', resize: 'vertical' }} />
                    </div>
            ...
            <div className="animate-fade-in" style={{ padding: '0.5rem', display: 'flex', flexDirection: 'column', height: '100%' }}>
                ...
                <div style={{ flex: 1, overflowY: 'auto' }}>
                    ...
                    <div style={{ marginBottom: '2rem' }}>
                        ...
                        <p style={{ fontSize: '0.9rem', color: '#cbd5e1', lineHeight: 1.6, whiteSpace: 'pre-wrap', background: '#0f172a', padding: '1rem', borderRadius: 8, border: '1px solid #334155' }}>{activeThread.content}</p>
                        
                        {activeThread.code_snapshot && (
                            <div style={{ marginTop: '1rem', borderRadius: 8, overflow: 'hidden', border: '1px solid #334155' }}>
                                <div style={{ padding: '0.5rem 1rem', background: '#0f172a', borderBottom: '1px solid #334155', fontSize: '0.75rem', color: '#94a3b8', display: 'flex', alignItems: 'center', gap: '0.5rem' }}><CodeIcon size={14} /> Attached Code Snapshot</div>
                                <pre style={{ margin: 0, padding: '1rem', background: '#1e293b', color: '#e2e8f0', fontSize: '0.8rem', overflowX: 'auto' }}>{activeThread.code_snapshot}</pre>
                            </div>
                        )}
                    </div>

                    {/* Replies */}
                    <h3 style={{ fontSize: '0.9rem', fontWeight: 700, color: '#94a3b8', marginBottom: '1rem', textTransform: 'uppercase' }}>Replies</h3>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '2rem' }}>
                        {replies.length === 0 ? (
                            <div style={{ textAlign: 'center', padding: '2rem', color: '#64748b', fontSize: '0.85rem', background: '#0f172a', borderRadius: 8, border: '1px dashed #334155' }}>No replies yet.</div>
                        ) : (
                            replies.map(reply => (
                ...
                {/* Reply Form */}
                <form onSubmit={handlePostReply} style={{ display: 'flex', gap: '0.5rem', marginTop: 'auto', paddingTop: '1rem', borderTop: '1px solid #334155' }}>
                    <input value={replyContent} onChange={e => setReplyContent(e.target.value)} placeholder="Type your reply..." style={{ flex: 1, padding: '0.75rem', borderRadius: 8, background: '#0f172a', border: '1px solid #334155', color: '#fff', fontSize: '0.9rem' }} />
        ...
        <div className="animate-fade-in" style={{ padding: '0.5rem', display: 'flex', flexDirection: 'column', height: '100%' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                {discussions.length === 0 ? (
                    ...
                    discussions.map(d => (
                        <div key={d.id} onClick={() => { setActiveThread(d); setView('thread') }} style={{ padding: '1rem', borderRadius: 8, background: '#0f172a', border: '1px solid #334155', cursor: 'pointer', transition: 'all 0.2s ease', ':hover': { borderColor: '#475569' } }}>
