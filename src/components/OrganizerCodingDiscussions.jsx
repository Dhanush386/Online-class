                ...
                <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', paddingRight: '1rem' }}>
                    ...
                    <div style={{ marginBottom: '2rem', padding: '1.5rem', background: '#f8fafc', borderRadius: 12, border: '1px solid #e2e8f0' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
                            <div>
                                <h2 style={{ fontSize: '1.2rem', fontWeight: 800, color: '#0f172a', marginBottom: '0.25rem' }}>{activeThread.title}</h2>
                        ...
                        <p style={{ fontSize: '0.95rem', color: '#334155', lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>{activeThread.content}</p>
                        
                        {activeThread.code_snapshot && (
                            <div style={{ marginTop: '1.5rem', borderRadius: 8, overflow: 'hidden', border: '1px solid #cbd5e1' }}>
                                <div style={{ padding: '0.5rem 1rem', background: '#e2e8f0', borderBottom: '1px solid #cbd5e1', fontSize: '0.75rem', color: '#475569', display: 'flex', alignItems: 'center', gap: '0.5rem', fontWeight: 600 }}><CodeIcon size={14} /> Attached Code Snapshot</div>
                                <pre style={{ margin: 0, padding: '1rem', background: '#f1f5f9', color: '#0f172a', fontSize: '0.85rem', overflowX: 'auto', border: 'none' }}>{activeThread.code_snapshot}</pre>
                            </div>
                        )}
                    </div>

                    {/* Replies */}
                    <h3 style={{ fontSize: '1rem', fontWeight: 700, color: '#64748b', marginBottom: '1rem' }}>Replies</h3>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '2rem' }}>
                        {replies.length === 0 ? (
                            <div style={{ textAlign: 'center', padding: '2rem', color: '#94a3b8', fontSize: '0.9rem', background: '#f8fafc', borderRadius: 8, border: '1px dashed #cbd5e1' }}>No replies yet.</div>
                        ) : (
                            replies.map(reply => (
                                <div key={reply.id} style={{ padding: '1.25rem', borderRadius: 12, background: reply.is_organizer ? '#eff6ff' : '#fff', border: `1px solid ${reply.is_organizer ? '#bfdbfe' : '#e2e8f0'}` }}>
                                    ...
                                    <p style={{ fontSize: '0.95rem', color: '#475569', lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>{reply.content}</p>
                                </div>
                            ))
                        )}
                    </div>
        ...
        <div className="glass-card animate-fade-in" style={{ padding: '1.5rem', minHeight: 400 }}>
            ...
            {loading ? (
                ...
                <div style={{ textAlign: 'center', padding: '4rem', background: '#f8fafc', borderRadius: 12, border: '1px dashed #cbd5e1' }}>
                    ...
                    <h3 style={{ fontSize: '1.1rem', fontWeight: 700, color: '#475569', marginBottom: '0.5rem' }}>No discussions found</h3>
                    <p style={{ color: '#94a3b8', fontSize: '0.9rem' }}>Students haven't asked any questions yet.</p>
                </div>
            ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    {filtered.map(d => (
                        <div key={d.id} onClick={() => setActiveThread(d)} style={{ padding: '1.25rem', borderRadius: 12, background: '#fff', border: '1px solid #e2e8f0', cursor: 'pointer', transition: 'all 0.2s', ':hover': { borderColor: '#94a3b8', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' } }}>
                            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '1rem', marginBottom: '0.5rem' }}>
                                <h4 style={{ fontSize: '1.05rem', fontWeight: 700, color: '#0f172a', margin: 0 }}>{d.title}</h4>
                                {d.status === 'resolved' ? (
